...
interface CreateFromNotionPageProps {
  onBack: () => void
...
export function CreateFromNotionPage({ onBack, onSuccess }: CreateFromNotionPageProps) {
  ...
  const handleConnectNotion = () => {
    if (notionUrl.trim()) {
      setIsConnected(true)
      // Auto-populate title from URL if possible
      const urlParts = notionUrl.split("/")
      const lastPart = urlParts[urlParts.length - 1]
      if (lastPart && !leadMagnetTitle) {
        const titleFromUrl = lastPart
          .replace(/-/g, " ")
          .replace(/[0-9a-f]{32}/, "")
          .trim()
        setLeadMagnetTitle(titleFromUrl)
      }
    }
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 relative">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Create Lead Magnet from Notion</h1>
      </div>
        ...
        <div className="w-full max-w-2xl space-y-6">
          ...
          <Card>
            <CardContent className="p-6">
              <h2 className="text-lg font-semibold mb-4">Connect Your Notion Page</h2>

              {!isConnected ? (
                <div className="space-y-4">
                  <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                    <BookOpen className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                    <h3 className="text-lg font-medium mb-2">Import from Notion</h3>
                    <p className="text-muted-foreground mb-4">
                      Enter the URL of your Notion page to import its content
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Make sure your Notion page is publicly accessible or shared with appropriate permissions
                    </p>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-green-600 truncate">{notionUrl}</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <div className="flex items-center gap-2 text-green-600">
                    ...
                    <span className="text-sm">Content imported successfully!</span>
                  </div>
                </div>
              )}
            </CardContent>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" id="import-text" defaultChecked />
                    <Label htmlFor="import-text">Import text content</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" id="import-images" defaultChecked />
                    <Label htmlFor="import-images">Import images</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" id="preserve-formatting" defaultChecked />
